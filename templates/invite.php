...
<div class="min-h-screen flex items-center justify-center px-4 py-10">
    <div class="w-full max-w-md">
        ...
        <div class="card p-6 shadow-lift">
            <h1 class="text-xl font-bold text-ink-900 flex items-center gap-2">
                <?= icon('mail', 'w-5 h-5 text-brand-700') ?>
                <?= h(t('Invite.title')) ?>
            </h1>
            <p class="text-sm text-ink-500 mt-2">
                <?= h(t('Invite.blurb', ['org' => $tenant['name'] ?? '—', 'email' => $invite['email']])) ?>
            </p>
            <form method="post" action="/invite/<?= h($invite['invite_token']) ?>" class="mt-6 space-y-4">
                <?= csrf_field() ?>
                <div>
                    <label class="label"><?= h(t('Register.fullName')) ?></label>
                    <div class="relative">
                        <span class="absolute left-3 top-1/2 -translate-y-1/2 text-ink-400"><?= icon('user', 'w-4 h-4') ?></span>
                ...
                <div>
                    ...
                    <div class="relative">
                        <span class="absolute left-3 top-1/2 -translate-y-1/2 text-ink-400"><?= icon('lock', 'w-4 h-4') ?></span>
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        ...
                        <div class="relative">
                            <span class="absolute left-3 top-1/2 -translate-y-1/2 text-ink-400"><?= icon('id-card', 'w-4 h-4') ?></span>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <span class="absolute left-3 top-1/2 -translate-y-1/2 text-ink-400"><?= icon('stethoscope', 'w-4 h-4') ?></span>
