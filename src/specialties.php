...
const SPECIALTIES = [
    [
        'id' => 'general',
        'name' => 'General Practice Agent',
        'specialty' => 'General Practice / Primary Care',
        'description' => 'Broad first-line primary-care reasoning across multiple systems. Useful when the presenting complaint is non-specific or ambulatory.',
        'icon' => 'stethoscope',
    ...
    [
        ...
        'description' => 'Adult inpatient and outpatient internal medicine focused on multimorbidity, chronic disease management, polypharmacy review, and hospitalist-level reasoning across organ systems.',
        'icon' => 'medical-cross',
        'system_prompt_addon' => "Build the problem list first (each active issue named and prioritized), then reason through each problem with explicit pretest probability (low/intermediate/high) and the next discriminating test. Integrate comorbidities: do NOT consider one organ system in isolation — every recommendation must account for renal function (eGFR), hepatic function (Child-Pugh / MELD when relevant), cardiac status (EF, NYHA), and frailty. Perform a structured medication review on every case: check for interactions, dose adjustments by renal/hepatic function, and inappropriate prescribing in older adults (Beers / STOPP-START). For inpatients, state a working disposition (floor / step-down / ICU / discharge) and apply NEWS2 / qSOFA for deterioration and sepsis. Anchor recommendations in current guidelines (USPSTF, ACP, NICE) without inventing citations — say 'per current guideline' if no specific citation was provided. Always identify time-critical conditions (sepsis, ACS, PE, GI bleed, stroke, hyperkalemia) explicitly.",
        'required_context' => [
            'Age, biological sex, and frailty / functional status (e.g. ECOG, Barthel)',
            'Active problem list and chronic conditions (HTN, DM, CKD, CHF, COPD, etc.)',
            'Full medication list including OTC, supplements, recent antibiotics',
            'Vital signs (BP, HR, RR, SpO2, temperature) with trend if inpatient',
            'Baseline and current labs: CBC, CMP, LFTs, INR, troponin/BNP if relevant, lactate, ABG/VBG if acute',
            'Allergies and prior adverse drug reactions',
            'Code status / advance directives when end-of-life issues are possible',
        ],
        'common_red_flags' => [
            'Sepsis: qSOFA ≥2, lactate ≥2, or organ dysfunction with infection',
            'Hemodynamic instability (SBP <90, shock index >1, signs of hypoperfusion)',
            'Acute respiratory failure (SpO2 <90% on RA, PaCO2 ≥50 with acidosis)',
            'Severe electrolyte derangement (K >6 or <2.5, Na <120 or >160, Ca >13)',
            'Acute kidney injury KDIGO ≥2 with anuria or fluid overload',
            'GI bleed with hemodynamic compromise or Hgb drop ≥2 g/dL',
            'New focal neurologic deficit or persistent GCS <15 (delirium vs stroke)',
            'Polypharmacy interaction with QTc prolongation, serotonin syndrome, or NSAID-induced AKI',
        ],
        'recommended_investigations' => [
            'CBC with differential, CMP, magnesium, phosphate',
            'Lactate, procalcitonin, blood cultures x2 if sepsis suspected',
            'Troponin and BNP/NT-proBNP when dyspnea or chest pain present',
            'Urinalysis with microscopy and urine electrolytes for AKI work-up',
            'ECG and CXR as baseline for acute admissions',
            'Targeted imaging (CT chest/abdomen, US) driven by the problem list',
            'TSH, B12, HbA1c on geriatric admissions or unexplained decline',
        ],
        'validated_tools' => [
            'NEWS2 (clinical deterioration)',
            'qSOFA / SOFA (sepsis screening and severity)',
            'CURB-65 / PSI (pneumonia disposition)',
            'HEART / TIMI / GRACE (chest pain and NSTE-ACS)',
            'Wells + PERC (PE), Geneva score',
            'Glasgow-Blatchford / Rockall (UGIB)',
            'CHA2DS2-VASc / HAS-BLED (AF anticoagulation)',
            'MELD-Na / Child-Pugh (liver disease)',
            'KDIGO AKI staging and CKD-EPI eGFR',
            'Charlson Comorbidity Index and Clinical Frailty Scale',
            'Beers criteria / STOPP-START (inappropriate prescribing in older adults)',
            'CAM (delirium screening)',
        ],
        'analysis_style' => 'Problem-list-driven, multimorbidity-aware, evidence-anchored. Each problem gets a pretest probability and a next discriminating test. Every recommendation factors renal, hepatic, cardiac, and frailty status.',
    ],
    [
        'id' => 'cardiology',
